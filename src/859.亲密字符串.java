...
class Solution {
    public boolean buddyStrings(String s, String goal) {
        if (s == null || s.length() == 0 || goal == null || goal.length() == 0) { // 其中一个为null或者空字符串
            return false;
        } else if (s.length() != goal.length()) { // 两个字符串长度不匹配
            return false;
        } else {
            int length = s.length();
            ...
            if (s.equals(goal)) {
                ...
                for (int i = 0; i < length; i++) {
                    if (tempSet.contains(sChars[i])) {
                        return true;
                    } else {
                        tempSet.add(sChars[i]);
                    }
                }
            } else {
                int counter = 0;
                ...
                for (int i = 0; i < length; i++) {
                    if (sChars[i] != goalChars[i]) {
                        ...
                        number = number ^ (sChars[i] ^ goalChars[i]);
                    }
                }
                if (number == 0) {
                    return true;
