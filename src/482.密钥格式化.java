...
class Solution {
    ...
    // string = S.charAt(length - count) + string;
    // if (length - count != 0 && count % K == 0) {
    // string = "-" + string;
    // }
    // count++;
    // }
    ...
    public String licenseKeyFormatting(String S, int K) {
        ...
        int firstIndex = getFirstIndexOfDemicalOrCharacter(chars);
        int count = 1;
        for (int i = length - 1; i >= 0; i--) {
            if (chars[i] != '-') {
                string = chars[i] + string;
                if (count++ % K == 0 && i != firstIndex) {
                    string = '-' + string;
                }
            }
    ...
    public int getFirstIndexOfDemicalOrCharacter(char[] chars) {
