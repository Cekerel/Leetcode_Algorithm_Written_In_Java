...
class Solution {
    public List<List<Integer>> largeGroupPositions(String s) {
        List<List<Integer>> largeGroupPoList = new ArrayList<>();
        ...
        while(pointer < charArray.length) {
            if (charArray[pointer] == charArray[pointer - 1]) {
                end = pointer;   
            } else {
                if (end - start >= 2) {
                    largeGroupPoList.add(Arrays.asList(start, end));
                }
                start = pointer;
            }
            pointer++;
        }
        if(end == charArray.length - 1 && end - start >= 2) {
            largeGroupPoList.add(Arrays.asList(start, end));
        }
        return largeGroupPoList;
    }
}
// @lc code=end
