...
class Solution {
    public int[] shortestToChar(String s, char c) {
        ...
        for (int i = 0; i < chars.length; i++) {
            if (chars[i] == c) {
                int pointer = i - 1;
                while ((pointer >= 0 && chars[pointer] != c)) {
                    distances[pointer] = distances[pointer] == 0 ? i - pointer : Math.min(distances[pointer], i - pointer);
                    pointer--;
                }
                pointer = i + 1;
                while ((pointer < chars.length && chars[pointer] != c)) {
                    distances[pointer] = pointer - i;
                    pointer++;
                }
            }
        }
